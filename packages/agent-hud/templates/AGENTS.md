...
2. Start, finish, or plan a piece of work → update `tasks` (todo/doing/done).
3. Are asked about progress → update the file first, then point the user to
   `/agent-hud`.
...
- `pages[]`: `id` (unique slug) and `path` required. `path` is the route as the
  ...
  `todo` | `doing` | `done` (default `done`).
- `relations[]`: `from`/`to` are page ids; `type` is `nav` | `data` | `auth` |
  `other` (default `nav`).
